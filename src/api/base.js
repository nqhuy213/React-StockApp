...
export const callAPI = async (endpoint, method = 'GET', params) => {
  ...
  if(params){
    if(method === 'GET'){ //Put the params into endpoint
      endpoint += '?' + objectToQueryString(params)
    }else{ //If the method is not GET, put params to request body
...
function objectToQueryString(obj) {
  return Object.keys(obj).map(key => obj[key].trim() !== ''? key + '=' + obj[key].trim() : '').join('&');
}